<form action="{{ route('register') }}" method="POST" class="flex flex-col gap-5">
  @csrf
  <div class="flex flex-col gap-1">
    <label class="form-label font-normal text-gray-900">
      Ел. пошта
    </label>
    <input class="input @error('email') border-danger @enderror" name="email" type="email" value="{{ old('email') }}">
    @error('email')
    <span class="text-xs text-danger flex">{{ $message }}</span>
    @enderror
  </div>

  <div class="flex flex-col gap-1">
    <label class="form-label font-normal text-gray-900">
      Пароль
    </label>
    <div class="input @error('password') border-danger @enderror" data-toggle-password="true">
      <input name="password" type="password" value="" data-gtm-form-interact-field-id="0">
      <button class="btn btn-icon" data-toggle-password-trigger="true" type="button">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
          class="size-4">
          <path stroke-linecap="round" stroke-linejoin="round"
            d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
          <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
        </svg>
      </button>
    </div>

    @error('password')
    <span class="text-xs text-danger flex">{{ $message }}</span>
    @enderror

  </div>

  <div class="flex flex-col gap-1">
    <label class="form-label font-normal text-gray-900">
      Підтвердити пароль
    </label>
    <div class="input @error('password_confirmation') border-danger @enderror" data-toggle-password="true">
      <input name="password_confirmation" type="password" value="">
      <button class="btn btn-icon" data-toggle-password-trigger="true" type="button">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
          class="size-4">
          <path stroke-linecap="round" stroke-linejoin="round"
            d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
          <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
        </svg>
      </button>
    </div>

    @error('password_confirmation')
    <span class="text-xs text-danger flex">{{ $message }}</span>
    @enderror

  </div>

  <button type="submit" class="btn btn-primary flex justify-center grow">
    Регистрація
  </button>
</form>